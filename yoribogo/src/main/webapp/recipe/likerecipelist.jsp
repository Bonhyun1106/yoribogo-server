	...
	<div class="container">
		...
		<div class="wrapper">
			...
			<c:forEach var='list' items='${recipe}'>
				...
				<div class="recipe">
					<a href="detail.do?no=${list.recipeNo}&memNo=${user.memNo}" >
						<div class="image">
							<div class="quality">
								<i class="fas fa-eye fa-2x"></i>${list.recipeViewCnt} 
								<i class="fas fa-heart fa-2x"></i>${list.recipeLikeCnt} 
								<i class="fas fa-level-up-alt fa-2x"></i>
								<c:choose>
							        <c:when test = "${list.recipeLevel eq 4936}">
							            매우 쉬움
							        </c:when>
							        
							        <c:when test = "${list.recipeLevel eq 4937}">
							            쉬움
							        </c:when>
							        
							        <c:when test = "${list.recipeLevel eq 4938}">
							            보통
							        </c:when>
							        
							        <c:when test = "${list.recipeLevel eq 4939}">
							            어려움
							        </c:when>
							        
							        <c:when test = "${list.recipeLevel eq 4940}">
							            매우 어려움
							        </c:when>
							        
					            </c:choose>
	           				</div>
							<img src="${pageContext.request.contextPath}${list.recipePhoto}" />
						</div>
						...
						<div class="body">
							<div class="mid">
								...
								<div class="date">
									<div class="dateWriter">By <span id="writer_id">${list.memId}</span></div>
									<div>
										<i class="fas fa-star"></i>${list.recipeGrade}/5
									</div>
									<time>
										<fmt:formatDate value="${list.recipeRegDate}"
											pattern="yy.MM.dd" />
									</time>
								</div>
							</div>
							...
							<div class="title">
								<h3>${list.recipeTitle}</h3>
							</div>
							<div class="content">
								<div class="content_inner">${list.recipeSummary}</div>
							</div>
				...
				success: function (result) {
					...
					for (let i = 0; i < recipe.length; i++) {
						...
						let date = moment(list.regDate).format("YY.MM.DD");
							...
							html +=	'<img src="' + path + list.recipePhoto + '"/>';
							html +=	'</div>';
							html +=	'<div class="body">';
							html +=	'<div class="mid">';
							html +=	'<div class="date">';
							html +=	'<div class="dateWriter">By <span id="writer_id">' + list.memId + '</span></div>';
							html +=	'<div>';
							html +=	'<i class="fas fa-star"></i>' + list.recipeGrade + '/5';
							html +=	'</div>';
							html +=	'<time>';
							html +=	date;
							html +=	'</time>';
							...
							html +=	'<h3>' + list.recipeTitle + '</h3>';
							html +=	'</div>';
							html +=	'<div class="content">';
							html +=	'<div class="content_inner">' + list.recipeSummary + '</div>';
							html +=	'</div>';
							html += '</div>';
							html += '</a>';
							html += '</div>';
